...
(function () {
  ...
  // DEFAULT_BASE usata dalle helper functions
  const DEFAULT_BASE = `https://${VIXSRC_URL}`;

  // PROXIES presi da index-wzrcuT8C.js
  ...
  // UTILITIES
  function log(...args) { if (StreamConfig.DEBUG && window && window.console) console.log("[script.js]", ...args); }
  ...
  function saveJSON(key, obj) { try { localStorage.setItem(key, JSON.stringify(obj)); } catch (e) { log("saveJSON failed", e); } }
  function loadJSON(key) { try { const s = localStorage.getItem(key); return s ? JSON.parse(s) : null; } catch (e) { return null; } }

  // NAVIGATION HELPER (fix per navigazione)
  ...
  async function findBestProxy(testTarget = DEFAULT_BASE + "/") {
  ...
  function applyCorsProxy(url) {
    try {
      ...
      // If resource is not under default base or origin, do not proxy by default
  ...
  async function fetchTVSeasons(tvId) {
    if (tvId === 87623) {
      return [{ season_number: 1, name: "Stagione 1" }, { season_number: 2, name: "Stagione 2" }, { season_number: 3, name: "Stagione 3" }];
    }
  ...
  function createCard(item, cookieNames = [], isRemovable = false) {
    ...
    const anno = (item.release_date && item.release_date.slice(0,4)) || (item.first_air_date && item.first_air_date.slice(0,4)) || "—";
    const voto = item.vote_average ? item.vote_average.toFixed(1) : "—";
    ...
    if (item.genres && item.genres.length > 0) genreText = item.genres.slice(0, 2).map(g => createGenreTag(g.name)).join("");
    else if (item.genre_ids && item.genre_ids.length > 0) genreText = item.genre_ids.slice(0, 2).map(id => GENRES[id]).filter(Boolean).map(name => createGenreTag(name)).join("");
      ...
      removeBtn.addEventListener("click", (e) => {
        ...
        if (confirmDelete) {
          ...
          if (container && container.children.length === 0) {
            const sec = document.getElementById("continua-visione");
  ...
  async function loadContinuaDaCookie() {
    ...
    for (const [key, info] of lastCookieByContent.entries()) {
      const { name, parts, type, tmdbId } = info;
  ...
  // UI HELPERS
  function goBack() { if (player) { try { player.dispose(); } catch(e){} player = null; } showSection('home'); }
  ...
  // Optional mobile enhancements placeholder
